...
def create_post(request, classId):
    if request.method == "POST":
        ...
        if json_files is not None:
            ...
            class_folder_name = classroom.group.tag + " - " + classroom.name
            try:
                class_user_folder = Folder.objects.get(author=request.session.user, name=folder_name, parent_folder=posts_docs_root_folder, is_autogenerated=True)
            except Folder.DoesNotExist:
...
def create_assignment_submit(request, assignmentId):
    if request.method == "POST":
        ...
        if json_files is not None:
            ...
            try:
                class_user_folder = Folder.objects.get(author=request.session.user, name=folder_name, parent_folder=submits_root_folder, is_autogenerated=True)
            except Folder.DoesNotExist:
                class_user_folder = Folder()
                class_user_folder.author = request.session.user
                class_user_folder.name = class_folder_name
