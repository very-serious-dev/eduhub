...
def admin_create_user(request):
    if request.method == "POST":
        ...
        return endpoints_admin.create_user(request, username, name, surname, password, student_group, is_teacher)
    else:
        raise Unsupported
...
def admin_edit_delete_user(request, username):
    if request.method == "PUT":
        ...
        if password is not None:
            validate_password(password)
        return endpoints_admin.edit_user(request, username, json_username, name, surname, password)
    elif request.method == "DELETE":
        return endpoints_admin.delete_user(request, username)
    else:
        raise Unsupported
...
def admin_create_group(request):
    if request.method == "POST":
        ...
        validate_tag(tag)
        validate_year(year)
        return endpoints_admin.create_group(request, tag, name, year, tutor_username)
    else:
        raise Unsupported
